...
function pix(payload = {}) {
  ...
  const code = [
    ...
    pad(Number(price).toFixed(2).length, 2), // Tamanho do valor da transação
    Number(price).toFixed(2), // Valor da transação com "." como separador decimal
    ...
    // Obrigatório = true
    '62', // Id do elemento
    pad(4 + String(transactionId).length, 2), /* Tamanho do próximo item: 0505 tem 4 caracteres + o item */
         // ID da transação
         /* */ '05', // Id do sub elemento
         /* */ pad(transactionId.length, 2), // Tamanho do código da transação
